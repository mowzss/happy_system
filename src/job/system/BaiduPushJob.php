...
class BaiduPushJob
{
    ...
    public function fire(Job $job, array $data): void
    {
        ...
        try {
            ...
            if ($result['success']) {
                // 推送成功，删除任务
                $job->delete();
                Log::info('百度推送成功: ' . $result['msg'] . ' URLs: ' . implode(', ', $urls));
            } else {
                // 推送失败，根据失败原因决定是否重试
                ...
                if ($attempts < $maxAttempts) {
                    ...
                    Log::warning("百度推送第 {$attempts} 次失败，准备重试: " . $result['msg']);
                } else {
                    // 超过最大重试次数，记录日志并删除任务
                    $job->delete();
                    Log::error('百度推送最终失败，已达到最大重试次数: ' . $result['msg']);
                }
            }
        } catch (\Exception $e) {
            $attempts = $job->attempts();
            if ($attempts < 3) {
                ...
                Log::warning("百度推送异常（第 {$attempts} 次）: " . $e->getMessage());
            } else {
                $job->delete();
                Log::error('百度推送异常最终失败: ' . $e->getMessage());
            }
        }
    }
    ...
    {
        Log::error('百度推送任务最终失败: ' . $exception->getMessage(), $data);
    }
