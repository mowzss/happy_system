...
class U2026061301
{
    ...
    {
        $system_config_group = new SystemConfigGroup();
        $config_group_data = [
            'title' => '手机版设置',
            'sys_show' => 1,
            'module' => 'system',
        ...
        $config_group_id = $system_config_group->save($config_group_data);
        if (!empty($config_group_id->id)) {
            SystemConfig::saveAll([
                [
                    'name' => 'site_wap_domain',
                    'type' => 'text',
                    'title' => '手机版域名',
                    'group_id' => $config_group_id->id,
                    'options' => '',
